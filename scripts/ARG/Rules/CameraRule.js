...
const tickFunction = () => {
    for (const player of GlobalVars.players) {
        ...
        if (block.block.typeId.includes("cobblestone_wall")) {
            player.setDynamicProperty("cameraSeconds", player.getDynamicProperty("cameraSeconds") + 1);
            world.sendMessage(`Seconds: ${player.getDynamicProperty("cameraSeconds")}`);
            if (player.getDynamicProperty("cameraSeconds") >= 5) {
